...
function main()
    ...
    for j in eachindex(bulk_data)
        ...
        # Extract metadata rows and write to metadata.csv
        if !isempty(row_indices)
            first_read_index = row_indices[1]
            metadata_df = df[1:first_read_index-1, :]
            CSV.write("$output_dir/metadata.csv", metadata_df)
        end
